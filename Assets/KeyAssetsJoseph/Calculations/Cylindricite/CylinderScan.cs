...
public class CylinderScan : MonoBehaviour
{
    //Basic stuff
    ...
    public List<VertexDat> selectedVerts = new List<VertexDat>();
    ...
    void Start()
    {
        ...
        manager.GetComponent<CalculationManager>().addObjectToList(manager.GetComponent<CalculationManager>().calculatorsCylindrical, transform.gameObject);
    ...
    public void ColorVerts()
    {
      ...
      Color finalColor;
        ...
        for (int i = 0; selectedVerts.Count > i; i++)
            {
                if (Mathf.Abs(selectedVerts[i].distance) > Tolerance) // La valeur est plus grande que celle toleree
                {
    ...
    {
        float biggestdistance= selectedVerts[0].distance;
        float newDist;
        for (int i = 0; selectedVerts.Count > i; i++)
        {
    ...
    public void DistanceMeasuring()
    {
        ...
        for (int i = 0; selectedVerts.Count > i; i++)
        {
            ...
            projectedPoint = plane.ClosestPointOnPlane(selectedVerts[i].vertPos);
            distanceToCol = Vector3.Distance(transform.position, projectedPoint);
            selectedVerts[i] = new VertexDat(selectedVerts[i].vertNum, selectedVerts[i].vertPos, distanceToCol );
    ...
    public void SelectVertices()
    {
        ...
        BoxCollider selectionCollider= transform.GetChild(0).GetChild(0).GetComponent<BoxCollider>();
        ...
        for (int i = 0; verts.Length > i; i++)
        {
            ...
            if (PointInsideCollider(worldPos,selectionCollider))
            {
    ...
    bool PointInsideCollider(Vector3 point,BoxCollider selCollide)
    {
        
        Vector3 closest = selCollide.ClosestPoint(point);
